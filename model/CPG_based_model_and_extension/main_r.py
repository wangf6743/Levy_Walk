# This is the main part of the fixed-r model. We iterate over fixed tent-map r values
# and run simulations for each value.
import argparse
import os
import time
...
DEFAULT_OUTPUT_DIR = SCRIPT_DIR / "data_fixr"
...
def dist_wall(x: np.ndarray, radius: float) -> Union[np.ndarray, float]:
...
def gait_generation(
    ...
    output_cpg = coupling_cpg @ f_tent(input_cpg, fixed_r)
    ...
    return output_angle, output_location, output_cpg, output_sc, dif_angle
...
def simulate_fixed_r(
    ...
    novelty = 1.0
    security = 1.0
    mental_r_history = np.full(step_number + 1, fixed_r, dtype=float)
...
def fixed_r_values(start: float = 0.0, stop: float = 0.5, step: float = 0.01) -> List[float]:
...
def run_simulation(
    ...
    r_values = fixed_r_values()
    max_workers = workers if workers is not None else min(len(r_values), os.cpu_count() or 1)
    ...
    for trial_idx in range(trialnumber):
        ...
        if max_workers == 1:
            trial_tracks = [
                ...
                for fixed_r in r_values
            ]
        else:
            print(f"  Running {len(r_values)} fixed r values with {max_workers} workers")
            with ProcessPoolExecutor(max_workers=max_workers) as executor:
                trial_tracks = list(
                    executor.map(
                        simulate_fixed_r,
                        r_values,
                        [initial_x] * len(r_values),
                        [initial_y] * len(r_values),
                        [step_number] * len(r_values),
                    )
                )
...
def format_r_filename(r: float) -> str:
    r_text = f"{r:.2f}".replace("-", "minus_").replace(".", "p")
    return f"r_{r_text}.csv"
...
def save_csv_files(trackdata: List[List[Track]], output_dir: Path, start_trial: int = 1) -> None:
    for trial_idx, trial_tracks in enumerate(trackdata, start=start_trial):
        ...
        for track in trial_tracks:
            ...
            output_path = trial_dir / format_r_filename(track.r)
            np.savetxt(
                output_path,
                csv_data,
                delimiter=",",
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run simulations with fixed tent-map r values.")
    parser.add_argument("--trials", type=int, default=100, help="number of trials")
    parser.add_argument("--start-trial", type=int, default=51, help="starting trial index for output folders")
    parser.add_argument("--steps", type=int, default=40_000, help="steps per fixed r value")
    parser.add_argument("--seed", type=int, default=None, help="random seed")
    parser.add_argument(
        "--workers",
        type=int,
        default=None,
        help="parallel workers for r values within each trial; default uses up to 51 CPUs",
    )
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=DEFAULT_OUTPUT_DIR,
        help="directory for saving trial folders and fixed-r CSV files",
    )
